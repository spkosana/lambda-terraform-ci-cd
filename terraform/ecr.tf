...
resource "aws_ecr_repository" "users" {
  name                 = "${terraform.workspace}-users-generator"
  ...
  tags = {
    Name = "Users generator ECR repo"
  }

}


# aws ecr get-login-password --region us-east-2 | docker login --username AWS --password-stdin 572903848524.dkr.ecr.us-east-2.amazonaws.com
